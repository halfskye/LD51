Shader "UniStorm/Celestial/Skybox" {
Properties {
	...
	_NightSkyTint ("Night Sky Tint", Color) = (.5, .5, .5, 1)

	_Exposure("Exposure", Range(0, 8)) = 1.3
	
	_CameraHeight ("Camera Height", Range(0, 360)) = 0
	
	_Rotation ("Rotation", Range(0, 360)) = 0
	_RotationEye ("Rotation", Range(0, 360)) = 0
...
SubShader {
	...
	Pass {
		...
		v2f vert (appdata_t v)
		{
			...
			float3 cameraPos = float3(0,kInnerRadius + _CameraHeight,0); 	// The camera's current position

			// Get the ray from the camera to the vertex and its length (which is the far point of the ray passing through the atmosphere)
			
			float3 eyeRay = normalize(mul((float3x3)unity_ObjectToWorld, v.vertex.xyz));
			eyeRay = mul( rotationMatrix( normalize(_RotationAxis.xyz), _RotationEye * UNITY_PI / 180.0), eyeRay).xyz;
			...
			*/
			...
			else
			{
				...
				float height = kInnerRadius + _CameraHeight;
				float depth = exp(kScaleOverScaleDepth * (-_CameraHeight));
